...
resource "proxmox_vm_qemu" "my_vm" {
 count       = var.vm_count
 name = "${var.aws_instance_type}_${var.vm_name_prefix}_${var.vm_count}_${count.index + 1}" # Nombra las VMs de manera secuencial t2.micro_aws-instance_100
 desc        = "${vm_clone_template}-${count.index + 1}"
